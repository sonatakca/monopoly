...
export default function SellHouseModal({
  open,
  tiles,
  houses,
  onSelect,
      ...
      <div style={{ background: 'rgba(17,24,39,0.8)', border: '1px solid rgba(255,255,255,0.14)', color: '#fff', borderRadius: 12, width: 520, maxWidth: '96vw', boxShadow: '0 14px 48px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 360, overflow: 'auto' }}>
          {list.length ? list.map((id) => {
            ...
            return (
              <div key={id}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
                  <div style={{ fontWeight: 800, fontSize: 14, lineHeight: '18px' }}>{name}</div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    {Array.from({ length: 4 }).map((_, i) => {
                      const key = `${id}-${i}`
                      const active = i < (houseCount?.[id] || 0)
                      ...
                      return (
                        <img
                          ...
                          style={{ width: 22, height: 'auto', opacity, cursor: 'pointer', transition: 'opacity 120ms ease' }}
                        />
                      )
                    })}
                  </div>
            ...
            <div style={{ opacity: 0.8 }}>Satılabilir ev yok</div>
          )}
        </div>
